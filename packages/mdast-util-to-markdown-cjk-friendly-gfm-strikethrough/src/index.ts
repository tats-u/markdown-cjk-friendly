import type { Delete, Parents, Text } from "mdast";
import type { Info, Options, State, Unsafe } from "mdast-util-to-markdown";
import {
  classifyCharacter,
...
const constructsWithoutStrikethrough = [
  ...
  "titleQuote",
  "titleApostrophe",
] satisfies Unsafe["notInConstruct"];
 ...
 */
export function cjkFriendlyGfmStrikethroughToMarkdown(): Options {
  return {
    unsafe: [
      {
        character: "~",
...
function handleDelete(
  ...
  const sequence = "~~";
  const exit = state.enter("strikethrough" as never);
...
function encodeInfoCjk(
  ...
  if (!isCjkOrIvs(beforeKind) && !isCjkOrIvs(afterKind)) {
    return target === "open"
      ? encodeInfoFallback(beforeKind, afterKind)
      : encodeInfoFallback(afterKind, beforeKind);
  }
  ...
  for (const candidate of [raw, preserveOutside, preserveInside, encodeBoth]) {
    ...
    const forms =
      target === "open"
        ? canOpen(candidateBefore, candidateAfter)
        : canClose(candidateBefore, candidateAfter);
...
function canOpen(before: BoundaryContext, afterCode: number | null): boolean {
  const beforeKind = classifyBoundaryBefore(before);
  const { open } = getAttentionSides(beforeKind, classifyCharacter(afterCode));

  return open;
}

function canClose(before: BoundaryContext, afterCode: number | null): boolean {
  const afterKind = classifyCharacter(afterCode);
  const { close } = getAttentionSides(
