...
describe("cjkFriendlyGfmStrikethroughToMarkdown", () => {
  const marker = "~~";
  const srcList = (
    [
      ["漢", true, "()", true, "字"],
  ...
  ).map(([before, canOpen, content, canClose, after]) => ({
    after,
    before,
    canClose,
    canOpen,
  ...
  it.each(
    srcList,
  )("Handles surrounding characters around delete ($before + $marker + $content + $marker + $after) (canOpen: $canOpen, canClose: $canClose)", ({
    before,
    canOpen,
    content,
    canClose,
