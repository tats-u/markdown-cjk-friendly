...
describe("index.html (pages)", () => {
  ...
  beforeAll(async () => {
    const source = await loadHtml("../.vitepress/dist/index.html");
    // biome-ignore lint/style/noNonNullAssertion: asserted in the next line
    markdown = new JSDOM(source).window.document.querySelector(".vp-doc")!;
  ...
  it("contains <del>", () => {
    // Vitepress uses <s> instead of <del>
    expect(markdown.querySelector("s")).not.toBeNull();
  });
